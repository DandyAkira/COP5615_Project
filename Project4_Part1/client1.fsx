...
let Client (actorName:string) (userName:string) (password:string) (mailbox: Actor<_>) =
    ...
    let accountMessage = {UserName=userName; Password=password; IPAndPort=clientIP+":8000"; ActorName=actorName}
    let rec loop () = actor {
...
for eachClient in clientList do
    let id = eachClient.Path.Name.Substring 7
    if (int id) > 1 then
        eachClient <! {SubscribeeName = "user_"+ (string ((int id)-1))} // subscribe to former one
    Thread.Sleep(50)

for eachClient in clientList do
    let id = eachClient.Path.Name.Substring 7
    eachClient <! "user_" + id + " saying hello to twitter @user_" + (string ((int id)-1))+ " "
    Thread.Sleep(50)
